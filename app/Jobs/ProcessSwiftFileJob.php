...
use App\Models\SwiftMessageDetail;                          // ← AJOUT IA
...
class ProcessSwiftFileJob implements ShouldQueue
{
    ...
    private function detectAndNormalizeType(SimpleXMLElement $xml, string $defaultNs, string $rawXml): string
    {
        ...
        foreach (['pacs.008', 'pacs.009', 'camt.053', 'camt.054', 'pain.001'] as $t) {
            if (str_contains($lowerNs, $t)) {
                return strtoupper($t);
            }
        }
        foreach (['pacs.008', 'pacs.009', 'camt.053', 'camt.054', 'pain.001'] as $t) {
            if (str_contains($lowerRaw, $t)) {
                return strtoupper($t);
            }
        }
        ...
        if (str_contains($root, 'ficstmrcdttrf') || str_contains($root, 'fitoficstmrcdttrf')) {
            return 'PACS.008';
        }
        if (str_contains($root, 'ficdttrf')) {
            return 'PACS.009';
        }
        if (str_contains($root, 'bktocstmrstmt')) {
            return 'CAMT.053';
        }

        if (preg_match('/<MT(\d{3})/i', $rawXml, $m)) {
            return 'MT'.$m[1];
        }
    ...
    private function determineCategory(string $typeShort): string
    {
        ...
        if (str_starts_with($upper, 'PACS')) {
            return 'PACS';
        }
        if (str_starts_with($upper, 'CAMT')) {
            return 'CAMT';
        }
        if (str_starts_with($upper, 'PAIN')) {
            return 'PAIN';
        }
        if (preg_match('/^MT(\d)/', $upper, $m)) {
            return $m[1];
        }
