...
class SyncTransactionsFromMessages extends Command
{
    ...
    public function handle(): int
    {
            ...
            if ($amount <= 0 || !$currency) {
                ...
                if ($tag32A) {
                    ...
                    if ($parsedAmt > 0) {
                        ...
                        if (!$dryRun) {
                            $updatePayload = [];
                            if ($parsedAmt && !($message->AMOUNT ?? $message->amount))     $updatePayload['AMOUNT']     = $parsedAmt;
                            if ($parsedCcy && !($message->CURRENCY ?? $message->currency)) $updatePayload['CURRENCY']   = $parsedCcy;
                            if ($parsedDate && !($message->VALUE_DATE ?? $message->value_date)) $updatePayload['VALUE_DATE'] = $parsedDate;
        ...
        if ($fixed  > 0) $this->info(" 🔧 Montants corrigés : {$fixed}");
        if ($skipped > 0) $this->line(" ⏭ Ignorées : {$skipped}");
        if ($errors  > 0) $this->error(" ❌ Erreurs  : {$errors}");
        $this->info('══════════════════════════════════════');
